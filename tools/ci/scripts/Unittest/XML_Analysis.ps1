...
[xml]$xmlData = Get-Content -Path $FilePath
   ...
   if ($totalAssertsFailures -gt 0) {
        Write-Output "Error: There are failed assertions."
        exit -1
    }

    if ($skippedTestcases -gt 0) {
        Write-Output "Error: There are skipped test cases."
        exit -1
    }

    if ($totalAborted -gt 0) {
        Write-Output "Error: There are aborted tests."
        exit -1
    }

    if ($totalTestcases -ne $successfullTestcases) {
        Write-Output "Error: Not all test cases were successful."
        exit -1
    }

    if ($totalAsserts -ne $totalAssertsPassed) {
        Write-Output "Error: Not all assertions passed."
        exit -1
    }
