    ...
    # start build process
    $Build = Start-Process -FilePath "$ToolPath\$ToolVersion\Bin-en.\BR.AS.Build.exe" -ArgumentList "$Parent\TestObject\Ersa_Maschine.apj -c $Project -o $BinaryDirectory\Binaries -t $BinaryDirectory\Temp -all -buildRUCPackage" -RedirectStandardOutput "$BinaryDirectory\$Project.txt" -WindowStyle Hidden -Wait -PassThru
